...
func (el *MenuButton) Bind(binding string, callback command.BindCallback) {
...
func (el *MenuButton) UnBind(binding string) {
	if ok := tk.Binding.MatchString(binding); !ok {
		log.Error(fmt.Errorf("invalid binding: %s", binding))
		return
	}
...
func (el *MenuButton) GenerateEvent(event string) {
