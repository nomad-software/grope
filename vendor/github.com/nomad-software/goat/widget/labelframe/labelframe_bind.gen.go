...
func (el *LabelFrame) Bind(binding string, callback command.BindCallback) {
...
func (el *LabelFrame) UnBind(binding string) {
	if ok := tk.Binding.MatchString(binding); !ok {
		log.Error(fmt.Errorf("invalid binding: %s", binding))
		return
	}
...
func (el *LabelFrame) GenerateEvent(event string) {
