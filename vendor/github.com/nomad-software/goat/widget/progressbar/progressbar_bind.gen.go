...
func (el *ProgressBar) Bind(binding string, callback command.BindCallback) {
...
func (el *ProgressBar) UnBind(binding string) {
	if ok := tk.Binding.MatchString(binding); !ok {
		log.Error(fmt.Errorf("invalid binding: %s", binding))
		return
	}
...
func (el *ProgressBar) GenerateEvent(event string) {
