...
func (el *PanedWindow) Bind(binding string, callback command.BindCallback) {
...
func (el *PanedWindow) UnBind(binding string) {
	if ok := tk.Binding.MatchString(binding); !ok {
		log.Error(fmt.Errorf("invalid binding: %s", binding))
		return
	}
...
func (el *PanedWindow) GenerateEvent(event string) {
