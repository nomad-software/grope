...
// transparent.
func (el *Polygon) SetOutlineDash(first, second float64, others ...float64) {
	otherStr := ""
...
func (el *Polygon) SetHoverOutlineDash(first, second float64, others ...float64) {
	otherStr := ""
	for _, i := range others {
		otherStr += fmt.Sprintf(" %v", i)
	}
...
func (el *Polygon) SetDisabledOutlineDash(first, second float64, others ...float64) {
	otherStr := ""
	for _, i := range others {
		otherStr += fmt.Sprintf(" %v", i)
	}
...
func (el *Polygon) SetOutlineDashOffset(offset float64) {
