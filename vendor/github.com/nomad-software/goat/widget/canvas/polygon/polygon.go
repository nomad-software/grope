...
func (el *Polygon) SetCoords(x1, y1, x2, y2, x3, y3 float64, others ...float64) {
	otherStr := ""
	for _, i := range others {
		otherStr += fmt.Sprintf(" %v", i)
	}
	tk.Get().Eval("%s coords %s [list %v %v %v %v %v %v %s]", el.GetParent().GetID(), el.GetID(), x1, y1, x2, y2, x3, y3, otherStr)
}
